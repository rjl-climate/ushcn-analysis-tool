...
def load_analysis_results():
    ...
    base_path = Path("/Users/richardlyon/dev/mine/python/ushcn-heatisland/output/adjustment_bias_investigation")
    
    # Load heat island reports
    raw_report_path = base_path / "01_raw_data_analysis" / "simple_min_heat_island_report.json"
    tobs_report_path = base_path / "02_tobs_adjusted_analysis" / "simple_min_heat_island_report.json"  
    fls52_report_path = base_path / "03_fully_adjusted_analysis" / "simple_min_heat_island_report.json"
...
def create_comparison_table():
    ...
    df['uhii_percent_change'] = (df['uhii_change_from_raw'] / df.iloc[0]['uhii_celsius']) * 100
...
def create_uhii_progression_chart(df):
    ...
    for bar, value in zip(bars, uhii_values):
        ...
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.01,
                f'{value:.3f}°C', ha='center', va='bottom')
    
    # Line chart showing progression
    ax2.plot(range(len(datasets)), uhii_values, 'o-', linewidth=2, markersize=8, color='darkred')
...
def main():
    ...
    output_dir = Path("/Users/richardlyon/dev/mine/python/ushcn-heatisland/output/adjustment_bias_investigation/04_comparative_analysis")
    ...
    print(f"TOBs Adjusted UHII:      {df.iloc[1]['uhii_celsius']:.3f}°C ({analysis['adjustment_impacts']['tobs_impact']['uhii_change']:+.3f}°C, {analysis['adjustment_impacts']['tobs_impact']['percent_change']:+.1f}%)")
    print(f"Fully Adjusted UHII:     {df.iloc[2]['uhii_celsius']:.3f}°C ({analysis['adjustment_impacts']['additional_adjustments']['uhii_change']:+.3f}°C, {analysis['adjustment_impacts']['additional_adjustments']['percent_change']:+.1f}%)")
    print(f"Net Adjustment Impact:    {analysis['adjustment_impacts']['net_impact']['uhii_change']:+.3f}°C ({analysis['adjustment_impacts']['net_impact']['percent_change']:+.1f}%)")
    print(f"Statistical Significance: Raw={analysis['significance_preservation']['raw_significant']}, TOBs={analysis['significance_preservation']['tobs_significant']}, FLS52={analysis['significance_preservation']['fls52_significant']}")
    print(f"Data Quality Improvement: +{analysis['data_quality']['net_station_gain']} stations")
