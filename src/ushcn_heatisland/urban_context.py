...
class UrbanContextManager:
    ...
    def __init__(self, static_cities_path: Optional[Path] = None):
        """
        ...
        if static_cities_path is None:
            static_cities_path = Path(__file__).parent.parent.parent / "data" / "cities" / "us_cities_static.csv"
    ...
    def load_cities_data(self, min_population: int = 50000) -> gpd.GeoDataFrame:
        ...
        if not self.static_cities_path.exists():
            raise FileNotFoundError(f"Static cities database not found: {self.static_cities_path}")
    ...
    def load_urban_areas(self, min_population: int = 100000) -> gpd.GeoDataFrame:
        ...
        urban_areas = urban_areas[['urban_area_name', 'geometry']].copy()
    ...
    def classify_stations_urban_rural(
        ...
        if len(cities_gdf) == 0:
            warnings.warn("No cities data available for classification")
        ...
        if len(city_coords) > 0:
            ...
            classified_stations['nearest_city_name'] = cities_gdf.iloc[nearest_city_indices]['city_name'].values
            classified_stations['nearest_city_population'] = cities_gdf.iloc[nearest_city_indices]['population'].values
            
            # Apply classification logic
            if use_4_level_hierarchy:
                self._apply_4_level_classification(classified_stations, cities_gdf, distances_km, nearest_city_indices)
            else:
                # Legacy 3-level classification
                urban_mask = nearest_distances <= 50.0
                suburban_mask = (nearest_distances > 50.0) & (nearest_distances <= 100.0)
                
                classified_stations.loc[urban_mask, 'urban_classification'] = 'urban'
                classified_stations.loc[suburban_mask, 'urban_classification'] = 'suburban'
        ...
        if len(urban_areas_gdf) > 0:
            try:
                ...
                stations_with_urban = gpd.sjoin(
                    classified_stations, urban_areas_gdf, 
                ...
                if use_4_level_hierarchy:
                    ...
                    suburban_in_urban = (classified_stations['urban_classification'] == 'suburban') & urban_area_mask
                    classified_stations.loc[suburban_in_urban, 'urban_classification'] = 'urban_fringe'
                else:
                    # Legacy behavior: upgrade to urban_core
                    urban_area_mask = classified_stations['within_urban_area']
                    classified_stations.loc[urban_area_mask, 'urban_classification'] = 'urban_core'
                
            except Exception as e:
                warnings.warn(f"Could not perform urban area classification: {str(e)}")
    ...
    def _apply_4_level_classification(
        ...
        for i in range(n_stations):
            station_distances = distances_km[i, :]  # Distances from this station to all cities
            ...
            classified_stations.iloc[i, classified_stations.columns.get_loc('urban_classification')] = classification
    ...
    def get_urban_context_summary(
        ...
        classification_counts = classified_stations_gdf['urban_classification'].value_counts()
        
        # Calculate proximity statistics
        valid_distances = classified_stations_gdf['distance_to_nearest_city_km'].dropna()
        ...
        summary = {
            ...
            # Distance statistics
            'mean_distance_to_city_km': float(valid_distances.mean()) if len(valid_distances) > 0 else np.nan,
            'median_distance_to_city_km': float(valid_distances.median()) if len(valid_distances) > 0 else np.nan,
            'min_distance_to_city_km': float(valid_distances.min()) if len(valid_distances) > 0 else np.nan,
            'max_distance_to_city_km': float(valid_distances.max()) if len(valid_distances) > 0 else np.nan,
            
            # Population statistics
            'city_population_range': {
                'min': int(cities_gdf['population'].min()) if len(cities_gdf) > 0 else 0,
                'max': int(cities_gdf['population'].max()) if len(cities_gdf) > 0 else 0,
                'mean': int(cities_gdf['population'].mean()) if len(cities_gdf) > 0 else 0
            },
